...
class Session:
    ...
    def get_dataloader(self, dataset_name, img_dir, train_mode=True):
        dataset = {True: TrainValDataset, False: TestDataset}
        if train_mode:
        ...
        if train_mode:
            ...
            return self.dataloaders[dataset_name], dataset
...
def run_train_val(ckp_name='latest'):
    ...
    dt_train, _ = sess.get_dataloader(sess.train_data_path)
    dt_val, _ = sess.get_dataloader(sess.train_data_path)
...
def run_test(ckp_name):
    ...
    for i, (batch, rainy_fname) in enumerate(zip(dt, ds.mat_files)):
